...
class TestContextManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        class Radio(StatefulObject):
            ...
            def on_action(self, context):
                assert context == 'ctx1'
                assert self.ctx1 is True
                assert self.ctx2 is True
    ...
    def test_manager(self):
        ...
        assert radio.ctx1 is False
        assert radio.ctx2 is False
        radio.flick()
        assert radio.ctx1 is False
        assert radio.ctx2 is False
        radio.flick()
...
class TestStateConstraint(unittest.TestCase):
    ...
    def test(self):
        ...
        user.login(password='wrong')
        assert user.state == 'active.logged_out'
        user.login(password='very_secret')
        assert user.state == 'active.logged_in'


class TestStateConstraint2(unittest.TestCase):
    ...
    def test(self):
        user = self.user_class('rosemary').activate(password='secret')
